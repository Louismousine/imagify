...
public class ImageController {
    ...
    // Search images by labels which were generated by Amazon Rekognition.
    @GetMapping("/findSimilar/{id}")
    public ResponseEntity<List<ImageDTO>> searchByTags(@PathVariable("id") Long id, @RequestHeader String token) throws TokenException {
        User user = userService.validateApiToken(token);
        ImageEntity ie = imageRepository.findImageEntityById(id);
        if (ie == null || ie.isPrivate() && !user.getUserName().equals(ie.getOwner().getUserName())) {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
        List<ImageDTO> imagesToReturn = new ArrayList<>();
        Iterable<ImageEntity> iterator = imageRepository.findAll();
        iterator.forEach(e -> {
            if (!e.isPrivate() || user.getUserName().equals(e.getOwner().getUserName())) {
                for (String label : ie.getLabels()) {
                    if(e.getLabels().contains(label)){
                        imagesToReturn.add(ImageDTO.convertImageToDTO(e));
                        break;
                    }
                }
            }
